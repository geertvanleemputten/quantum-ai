...
public class JsoupRegistrationService {
    ...
    public void register() {
        ...
        final String url = "https://client.btreserve.cc/en-US/Account/Register";
        ...
        document.select("input[name=\"ConfirmPassword\"]").first().val("1234Abcd");
        document.select("input[name=\"checkbox\"]").forEach(x -> {
            x.attr("checked", "checked");
            x.attr("value", "true");
        });

        final Connection.Response registrationResponse = ((FormElement) document.select("form").first()).submit().followRedirects(true).execute();
        ...
        final boolean success = "https://client.btreserve.cc/".equals(registrationResponse.url().toString());
        if (success) {
            count++;
        }
